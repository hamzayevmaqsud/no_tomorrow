...
class Workout {
  ...
  Workout({required this.id, required this.name, required this.date, List<ExerciseSet>? sets})
      : sets = sets ?? [];
...
class _WorkoutCard extends StatelessWidget {
  ...
  const _WorkoutCard({required this.workout, required this.onCompleteSet, required this.onDelete});
    ...
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(16),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Icon(Icons.fitness_center_rounded, size: 14, color: AppColors.workouts),
            const SizedBox(width: 8),
            Expanded(child: Text(workout.name, style: GoogleFonts.playfairDisplay(
              fontSize: 18, fontWeight: FontWeight.w700, fontStyle: FontStyle.italic, color: textCol))),
            Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
              decoration: BoxDecoration(color: AppColors.gold.withAlpha(20), borderRadius: BorderRadius.circular(8)),
              child: Text('+${workout.xp} XP', style: GoogleFonts.jetBrainsMono(
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(duration: const Duration(milliseconds: 200),
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: s.done ? AppColors.workouts.withAlpha(80) : textCol.withAlpha(20))),
                    child: Text(s.done ? '✓ ${s.reps}×${s.weight}kg' : '${s.reps}×${s.weight}kg',
...
class _AddWorkoutSheetState extends State<_AddWorkoutSheet> {
  ...
  void _submit() {
    ...
    widget.onAdd(Workout(
      ...
      sets: List.generate(_setCount, (_) => ExerciseSet(reps: _reps, weight: _weight)),
    ));
  }
